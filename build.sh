...
cd "$(dirname "$0")"
GITHASH_MAIN=$(git show -s --format="%h" HEAD)
cd REAPERS
GITHASH_REAPERS=$(git show -s --format="%h" HEAD)
cd ..
GITHASH="${GITHASH_MAIN}:${GITHASH_REAPERS}"
COMMONOPTS="-std=c++20 -Wall -Wno-maybe-uninitialized -Wno-uninitialized -I${EIGEN_INC} -IREAPERS/inc"
...
NVCC="nvcc -forward-unknown-to-host-compiler $COMMONOPTS $NDBGOPTS -Wno-unknown-pragmas -lcublas -lcusolver -lcurand REAPERS/src/krnl.cu -march=native -fopenmp $LINKLIBS -arch sm_80"
